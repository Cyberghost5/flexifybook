        ...
        <div class="col-lg-6 col-12 ">
            <div class="">
                <dl class="mb-0">
                    ...
                    <div class="appointment-detail-item d-flex flex-wrap align-items-start gap-2">
                    <dt class="text-md mb-0 h6">{{ __('Payment Type:') }}</dt>
                    <dd class="text-md mb-0">
                        {{ !empty($appointment->payment_type) ? $appointment->payment_type : '-' }}
                    </dd>
                    </div>
                    @if(!empty($appointment->ServiceData) && $appointment->ServiceData->allow_partial_payment)
                    <div class="appointment-detail-item d-flex flex-wrap align-items-start gap-2">
                    <dt class="text-md mb-0 h6">{{ __('Payment Status:') }}</dt>
                    <dd class="text-md mb-0">
                        @php
                            $service = $appointment->ServiceData;
                            $paymentAmount = 0;
                            
                            // Get payment amount from payment record
                            ...
                            $isPartialPayment = $service && $service->allow_partial_payment && $paymentAmount == $service->partial_payment_amount;
                            $remainingAmount = $service ? $service->price - $paymentAmount : 0;
                        @endphp
                        @if($isPartialPayment && $remainingAmount > 0)
                            <span class="badge bg-warning">{{ __('Partial Payment') }}</span>
                        @else
                    ...
                    <div class="appointment-detail-item d-flex flex-wrap align-items-start gap-2">
                    <dt class="text-md mb-0 h6">{{ __('Deposit Paid:') }}</dt>
                    <dd class="text-md mb-0">
                        {{ currency_format_with_sym($paymentAmount, $appointment->created_by, $appointment->business_id) }}
                    </dd>
                    </div>
                    @endif
                    @if($isPartialPayment && $remainingAmount > 0)
                    <div class="appointment-detail-item d-flex flex-wrap align-items-start gap-2">
                    <dt class="text-md mb-0 h6">{{ __('Remaining Amount:') }}</dt>
                    <dd class="text-md mb-0">
                        <span class="text-danger font-weight-bold">{{ currency_format_with_sym($remainingAmount, $appointment->created_by, $appointment->business_id) }}</span>
                    </dd>
                    </div>
                    @endif
                    @endif
