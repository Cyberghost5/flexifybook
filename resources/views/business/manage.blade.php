                    ...
                    <ul class="nav nav-pills nav-fill information-tab" id="pills-tab" role="tablist">
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link @if (session('tab') and session('tab') == 13) active @endif"
                                id="terms-conditions-tab" data-bs-toggle="pill" data-bs-target="#terms-conditions"
                                type="button">{{ __('Terms & Conditions') }}</button>
                        </li>
                        {{-- PWA li  --}}
                        @stack('PWA_menu')
                    </ul>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="tab-content" id="pills-tabContent">
                                                    ...
                                                    @foreach ($services as $service)
                                                        <tbody>
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    @if($service->allow_partial_payment && $service->partial_payment_amount > 0)
                                                                        <span class="badge bg-success">{{ __('Enabled') }}</span><br>
                                                                        <small class="text-muted">
                                                                            {{ __('Deposit') }}: {{ currency_format_with_sym($service->partial_payment_amount, $service->created_by, $service->business_id) }}<br>
                                                                            {{ __('Remaining') }}: {{ currency_format_with_sym($service->price - $service->partial_payment_amount, $service->created_by, $service->business_id) }}
                                                                        </small>
                                                                    @else
                                                                        <span class="badge bg-secondary">{{ __('Full Payment Only') }}</span>
                                                                    @endif
                                                                </td>
                                                                <td>
                                                                    <div class="d-flex">
                        ...
                        <div class="tab-pane fade @if (session('tab') and session('tab') == 6) show active @endif"
                            ...
                            <div class="row business-hrs">
                                ...
                                <div class="col-md-4">
                                    <div class="card">
                                        <div class="card-header">
                                            <h5 class="mb-0">{{ __('All Business Holidays') }}</h5>
                                            <small class="text-muted">{{ __('Total') }}: {{ count($businessholidays) }}</small>
                                        </div>
                                        <div class="card-body" style="max-height: 500px; overflow-y: auto;">
                                            @if(count($businessholidays) > 0)
                                                <ul class="event-cards list-group list-group-flush w-100">
                                                    @foreach ($businessholidays->sortBy('date') as $businessholiday)
                                                        <li class="mb-3 list-group-item card border">
                                                            <div class="row align-items-center justify-content-between">
                                                                <div class="col-auto mb-0">
                                                                    <div class="d-flex align-items-center">
                                                                        <div class="theme-avtar badge p-2 px-3 
                                                                            @if(\Carbon\Carbon::parse($businessholiday->date)->isPast()) bg-secondary
                                                                            @elseif(\Carbon\Carbon::parse($businessholiday->date)->isToday()) bg-warning
                                                                            @else bg-primary @endif">
                                                                            <i class="ti ti-calendar"></i>
                                                                        </div>
                                                                        <div class="ms-3">
                                                                            <h6 class="m-0">
                                                                                <span class="fc-event-title text-dark">
                                                                                    {{ $businessholiday->title }}
                                                                                </span>
                                                                                @if(\Carbon\Carbon::parse($businessholiday->date)->isPast())
                                                                                    <small class="badge bg-secondary ms-2">{{ __('Past') }}</small>
                                                                                @elseif(\Carbon\Carbon::parse($businessholiday->date)->isToday())
                                                                                    <small class="badge bg-warning ms-2">{{ __('Today') }}</small>
                                                                                @else
                                                                                    <small class="badge bg-success ms-2">{{ __('Upcoming') }}</small>
                                                                                @endif
                                                                            </h6>
                                                                            <small class="text-muted">
                                                                                @if($businessholiday->end_date && $businessholiday->end_date != $businessholiday->date)
                                                                                    <i class="ti ti-calendar-event me-1"></i>
                                                                                    {{ \Carbon\Carbon::parse($businessholiday->date)->format('M d, Y') }} - {{ \Carbon\Carbon::parse($businessholiday->end_date)->format('M d, Y') }}
                                                                                    <br><small class="text-info">{{ \Carbon\Carbon::parse($businessholiday->date)->diffInDays(\Carbon\Carbon::parse($businessholiday->end_date)) + 1 }} {{ __('days') }}</small>
                                                                                @else
                                                                                    <i class="ti ti-calendar me-1"></i>
                                                                                    {{ \Carbon\Carbon::parse($businessholiday->date)->format('M d, Y') }}
                                                                                @endif
                                                                            </small>
                                                                        </div>
                                                                    </div>
                                                                ...
                                                                <div class="col-auto d-flex align-items-center">
                                                                    ...
                                                                    @permission('holiday delete')
                                                                        <div class="action-btn">
                                                                            {{ Form::open(['route' => ['business-holiday.destroy', $businessholiday->id], 'class' => 'm-0', 'id' => 'delete-form-' . $businessholiday->id]) }}
                                                                            @method('DELETE')
                                                                            <button type="button"
                                                                                class="btn btn-sm bg-danger align-items-center show_confirm"
                                                                                data-bs-toggle="tooltip" 
                                                                                data-bs-original-title="{{ __('Delete') }}"
                                                                                data-confirm="{{ __('Are you sure you want to delete this holiday?') }}"
                                            ...
                                            @else
                                                <div class="text-center py-4">
                                                    ...
                                                    <h6 class="text-muted">{{ __('No holidays defined') }}</h6>
                                                    <p class="text-muted small">{{ __('Create your first business holiday using the button above.') }}</p>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
                        ...
                        <div class="tab-pane fade @if (session('tab') and session('tab') == 13) active show @endif"
                            id="terms-conditions" role="tabpanel" aria-labelledby="terms-conditions-tab">
                            {{ Form::open(['route' => ['business.terms-conditions.update', $business->id]]) }}
                            @csrf
                            <div class="card">
                                <div class="card-header">
                                    <h5>{{ __('Business Terms & Conditions') }}</h5>
                                    <p class="text-muted">{{ __('Set up terms and conditions that customers must agree to before making appointments') }}</p>
                                </div>
                                <div class="card-body">
                                    <div class="form-group">
                                        <label for="terms_conditions" class="form-label">{{ __('Terms & Conditions') }}</label>
                                        <textarea class="form-control" name="terms_conditions" id="terms_conditions" rows="15" 
                                                placeholder="{{ __('Enter your business terms and conditions here...') }}">{{ $business->terms_conditions }}</textarea>
                                        <small class="form-text text-muted">
                                            {{ __('These terms will be displayed to customers during the appointment booking process with a mandatory checkbox.') }}
                                        </small>
                                    </div>
                                </div>
                                <div class="card-footer text-end">
    ...
    <script type="text/javascript">
        ...
        (function() {
            ...
            var calendar = new FullCalendar.Calendar(document.getElementById('calendar'), {
                ...
                events: {!! json_encode($calendarHolidays) !!},
            });
            calendar.render();
        })();
