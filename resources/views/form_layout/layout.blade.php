    ...
    <script>
        $(document).ready(function() {
            ...
            $('#serviceSelect').on('change', function() {
                ...
                // Update payment options if the function exists
                if (typeof window.updatePaymentOptions === 'function' && selectedService) {
                    window.updatePaymentOptions(selectedService);
                }
            });
            ...
            function validateNewUserFields() {
                ...
                // Handle both old contact field and new country code + contact structure
                var contact = '';
                var countryCodeField = $('#new-user select[name="country_code"]');
                var contactField = $('#new-user input[name="contact"]');
                
                if (countryCodeField.length && contactField.length) {
                    // New structure with country code dropdown
                    var countryCode = countryCodeField.val() || '';
                    var phoneNumber = contactField.val() || '';
                    contact = (countryCode + phoneNumber).trim();
                } else {
                    // Fallback to old structure
                    contact = ($('#new-user #contact').val() || '').trim();
                }

                // Validate phone number format (country code + phone number)
                var isValidContact = contact && /^\+[1-9]\d{0,2}\d{1,15}$/.test(contact) && contact.length >= 5;
            ...
            function validateGuestUserFields() {
                ...
                // Handle both old contact field and new country code + contact structure
                var contact = '';
                var countryCodeField = $('#guest-user select[name="country_code"]');
                var contactField = $('#guest-user input[name="contact"]');
                ...
                if (countryCodeField.length && contactField.length) {
                    ...
                    // Fallback to old structure
                    contact = ($('#guest-user #contact').val() || '').trim();
                }
