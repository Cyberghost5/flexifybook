...
class ServiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (Auth::user()->isAbleTo('service create')) {
            ...
            // Handle partial payment settings
            if (!isset($request->is_service_free)) {
                $service->allow_partial_payment = isset($request->allow_partial_payment) ? 1 : 0;
                if ($service->allow_partial_payment) {
                    $partialAmount = $request->partial_payment_amount ?? 0;
                    // Ensure partial amount doesn't exceed service price
                    if ($partialAmount >= $service->price) {
                        return redirect()->back()->with('error', __('Partial payment amount cannot be greater than or equal to service price.'));
                    }
                    $service->partial_payment_amount = $partialAmount;
                } else {
                    $service->partial_payment_amount = 0;
                }
            } else {
                $service->allow_partial_payment = 0;
                $service->partial_payment_amount = 0;
            }
