...
{
    /**
     * Check if a given date falls within any business holiday
     */
    private function isHolidayDate($date, $businessId, $createdBy)
    {
        ...
        foreach ($holidays as $holiday) {
            ...
            if ($appointmentDate->between($startDate, $endDate)) {
                return [
                    'is_holiday' => true,
                    'holiday_name' => $holiday->title,
                    'holiday_period' => $holiday->end_date && $holiday->end_date != $holiday->date 
    ...
    public function store(Request $request)
    {
        if (Auth::user()->isAbleTo('appointment create')) {
            ...
            // Check if appointment date is a business holiday
            $holidayCheck = $this->isHolidayDate($request->appointment_date, getActiveBusiness(), creatorId());
            if ($holidayCheck['is_holiday']) {
                return redirect()->back()->with('error', __('Cannot book appointment on :holiday_name (:period). Please select a different date.', [
                    'holiday_name' => $holidayCheck['holiday_name'],
                    'period' => $holidayCheck['holiday_period']
                ]));
            }
    ...
    public function appointmentFormSubmit(Request $request)
    {
        ...
        if (!empty($request->service) && !empty($request->staff) && !empty($request->appointment_date) && !empty($request->email) && !empty($request->business_id) && !empty($request->payment)) {
            ...
            // Check if business has terms and conditions and if they are accepted
            if (!empty($business->terms_conditions)) {
                if (empty($request->terms_accepted) || $request->terms_accepted != '1') {
                    $redirecturl = route("appointments.form", ["slug" => $business->slug, "appointment" => 'failed']);
                    return response()->json([
                        'status' => 'failed',
                        'message' => __('You must accept the terms and conditions to proceed with the appointment booking.'),
                        'url' => $redirecturl
                    ]);
                }
            }
            ...
            // Check if appointment date is a business holiday
            $holidayCheck = $this->isHolidayDate($request->appointment_date, $business->id, $business->created_by);
            if ($holidayCheck['is_holiday']) {
                $redirecturl = route("appointments.form", ["slug" => $business->slug, "appointment" => 'failed']);
                return response()->json([
                    'status' => 'failed',
                    'message' => __('Cannot book appointment on :holiday_name (:period). Please select a different date.', [
                        'holiday_name' => $holidayCheck['holiday_name'],
                        'period' => $holidayCheck['holiday_period']
                    ]),
                    'url' => $redirecturl
                ...
                if ($roles) {
                    $user = User::create(
                        [
                            ...
                            'mobile_no' => !empty($request->contact) && !empty($request->country_code) 
                                ? $request->country_code . $request->contact 
                                : (!empty($request->contact) ? $request->contact : null),
            ...
            if ($request->type == 'guest-user') {
                ...
                $Appointment->contact      = !empty($request->contact) && !empty($request->country_code) 
                    ? $request->country_code . $request->contact 
                    : (!empty($request->contact) ? $request->contact : null);
            }
            ...
            if (!empty($custom_field) && $custom_field == 'on' && !empty($request->values)) {
            ...
            if (isset($request->payment_option) && $request->payment_option === 'partial' && $service->allow_partial_payment) {
                $payment_amount = min($service->partial_payment_amount, $final_amount);
            }

            $payment = AppointmentPayment::create([
                'appointment_id' => $Appointment->id,
                'payment_type' => $Appointment->payment_type,
                'amount' => module_is_active('FlexibleHours', $business->id) && !empty($flexible_hour) ? $flexible_hour->price : $payment_amount,
                'payment_date' => now(),
                'business_id' => $business->id,
                'created_by' => $business->created_by,
            ]);
