...
class BusinessController extends Controller
{
    ...
    public function businessManage($id)
    {
        if (Auth::user()->isAbleTo('business update')) {
            ...
            // Get business holidays as objects for the list display
            $businessholidays = BusinessHoliday::where('business_id', $id)->where('created_by', creatorId())->get();
            
            // Format business holidays for FullCalendar
            $calendarHolidays = BusinessHoliday::where('business_id', $id)->where('created_by', creatorId())->get()
                ->flatMap(function ($holiday) {
                    $events = [];
                    $startDate = \Carbon\Carbon::parse($holiday->date);
                    $endDate = $holiday->end_date ? \Carbon\Carbon::parse($holiday->end_date) : $startDate;
                    
                    // For date ranges, create a single event spanning the entire range
            ...
            return view('business.manage', compact('business', 'locations', 'categories', 'services', 'staffes', 'businessholidays', 'calendarHolidays', 'business_url', 'subdomain_Ip', 'subdomainPointing', 'domainip', 'domainPointing', 'serverIp', 'subdomain_name', 'company_settings', 'files', 'custom_field', 'custom_fields', 'pwa_data'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function updateTermsConditions(Request $request, $id)
    {
        if (Auth::user()->isAbleTo('business update')) {
            ...
            if ($business->created_by != creatorId()) {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
            ...
            return redirect()->back()->with('success', __('Terms and conditions updated successfully.'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
