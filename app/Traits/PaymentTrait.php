...
trait PaymentTrait
{
    ...
    public function payThisAppointment($request, $Session  = false)
    {
        ...
        if (isset($request->payment_option) && $request->payment_option === 'partial' && $service->allow_partial_payment) {
            $service_price = min($service->partial_payment_amount, $service->price);
        }
        ...
        // Final partial payment check - apply after all module calculations
        if (isset($request->payment_option) && $request->payment_option === 'partial' && $service->allow_partial_payment) {
            // For modules that set final_amount or total_amount, calculate partial from that amount
            if (isset($request->final_amount) || isset($request->total_amount)) {
                $final_amount = $request->final_amount ?? $request->total_amount ?? $service_price;
                $service_price = min($service->partial_payment_amount, $final_amount);
            } else {
                $service_price = min($service->partial_payment_amount, $service_price);
            }
        }
