...
class AppointmentDataTable extends DataTable
{
    ...
    {
        $rowColumn = ['id', 'customer_id', 'staff_id', 'service_id', 'location_id', 'appointment_status', 'date_duration', 'payment_info'];
        ...
        $dataTable = (new EloquentDataTable($query))->addIndexColumn()
            ...
            ->addColumn('payment_info', function (Appointment $appointment) {
                // Get service data to check if partial payment was used
                ...
                } elseif ((module_is_active('CollaborativeServices') || module_is_active('CompoundService')) && !empty($appointment->payments($appointment->id))) {
                    $paymentAmount = $appointment->payments($appointment->id)->amount;
                }
                
                // Check if service allows partial payment and if partial was used
                if ($service && $service->allow_partial_payment && $service->partial_payment_amount > 0) {
                    $isPartialPayment = $paymentAmount == $service->partial_payment_amount;
                    $remainingAmount = $service->price - $paymentAmount;
                    
                    if (request()->has('action') && request('action') === 'export') {
                        if ($isPartialPayment && $remainingAmount > 0) {
                            return 'Partial - Paid: ' . currency_format_with_sym($paymentAmount, $appointment->created_by, $appointment->business_id) . ' / Remaining: ' . currency_format_with_sym($remainingAmount, $appointment->created_by, $appointment->business_id);
                        }
                        return 'Full Payment - ' . currency_format_with_sym($paymentAmount, $appointment->created_by, $appointment->business_id);
                    }
                    
                    if ($isPartialPayment && $remainingAmount > 0) {
                        return '<div class="payment-info">
                            <span class="badge bg-warning">' . __('Partial Payment') . '</span><br>
                            <small class="text-muted">' . __('Paid') . ': ' . currency_format_with_sym($paymentAmount, $appointment->created_by, $appointment->business_id) . '</small><br>
                            <small class="text-danger">' . __('Remaining') . ': ' . currency_format_with_sym($remainingAmount, $appointment->created_by, $appointment->business_id) . '</small>
                        </div>';
                    }
                }
                
                return '<div class="payment-info">
                    <span class="badge bg-success">' . __('Full Payment') . '</span><br>
                    <small class="text-muted">' . currency_format_with_sym($paymentAmount, $appointment->created_by, $appointment->business_id) . '</small>
                </div>';
            });
    ...
    public function getColumns(): array
    {
        $columns =  [
            ...
            Column::make('payment_info')->title(__('Payment Status')),
